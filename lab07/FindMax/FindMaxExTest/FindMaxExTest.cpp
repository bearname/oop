...
SCENARIO("FindMaxEx, default comparator")
{
	GIVEN("default comparassion")
	{
		...
		AND_WHEN("input vector of struct is not empty")
		{
			...
			AND_WHEN("struct/class hasn't an operator '<'")
			{
				THEN("Error	C2893	Failed to specialize function template 'unknown-type std::less<void>::operator ()(_Ty1 &&,_Ty2 &&)")
				{

					/*	
						proof of such behavior of the function
						std::vector<Vector2D> v;
						Vector2D maxVector;
						REQUIRE(FindMax(v, maxVector));
						*/
					REQUIRE(true);
				}
			}
